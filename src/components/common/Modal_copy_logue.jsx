...
function Modal_logue({ isOpen, onClose , fingerprint}) {
  ...
  useEffect(() => {
    ...
    const saveVisitor = async () => {
      ...
      try {
        const q = query(collection(db, 'logue_visitors'), where('visitor_id', '==', fingerprint));
        const querySnapshot = await getDocs(q);

        if (querySnapshot.empty) {
          await addDoc(collection(db, 'logue_visitors'), { 
  ...
  const handleSave = async () => {
    ...
    try {
      await addDoc(collection(db, 'logue_inputs'), {
        visitor_id: fingerprint,
    ...
    >
      ...
      >
        ...
        <p style={{
          ...
          서비스 준비중입니다! <br />
          많은 분들의 관심 덕분에 열심히 준비하고 있어요. <br />
          곧 멋진 모습으로 찾아뵙겠습니다! <br />
          서비스가 오픈되면 가장 먼저 소식을 전해드릴게요.
        </p>
